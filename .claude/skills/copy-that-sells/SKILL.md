...
- `references/swipe-file.md`. A shorter, format-tagged swipe file for a fast hit when you want two or three proven patterns without opening the full examples library.
- `references/anti-ai-rules.md`. The quick anti-AI companion to `self-edit.md`: a plain-English swap table (leverage → use, unlock → open, seamless → easy), before/after structural fixes, and three fast tests (So-What, read-aloud, 1995). Use it to fix a flagged line fast. It is not a banned list — `self-edit.md` (prose) and `references/banned-patterns.txt` (machine) own that, so this file cannot drift from them.
...
**Templates (optional scaffolds, not straitjackets):**
- `templates/`. Fill-in blueprints for the six most-requested formats: `headline.md`, `tagline.md`, `billboard.md`, `landing-page.md`, `email.md`, `manifesto.md`. Reach for one when you want a fast structural start; the workflow above still governs. `references/formats.md` remains the deeper per-format reference (word counts, hook banks, RSA and App Store specifics).
...
### Automated check (optional backstop, not a substitute for reading aloud)

When a Python runtime is available, run the shippable copy through the validator before you hand it over:

```bash
python3 scripts/validate.py "Your headline or line here" --format headline
```

It flags banned phrases, corporate jargon, unproven superlatives, passive voice, missing specificity, and over-length headlines, and prints an Anti-AI Score out of 10. It reads its banned English **and Spanish** patterns from `references/banned-patterns.txt`, the single source of truth shared with the repo's `evals/check_banned_words.py` gate, so the two never disagree. Treat it as a smoke alarm, not a judge: a high score does not make weak copy good, and it scans one line at a time (pass the headline, then key lines), not whole paragraphs. Formats accept `headline`, `billboard`, `ooh`, `tagline`, `digital`, `general`. The `tests/` folder documents its behaviour; `./tests/run-tests.py` proves the scoring still holds. `references/self-edit.md` remains the authority; the script only catches the obvious tells before they ship.
