...
items=json.load(open('audio/v3/vo.json'))
only=set(sys.argv[1:])
for i,it in enumerate(items):
    if only and it['id'] not in only: continue
    body={"text":it['text'],"model_id":"eleven_multilingual_v2",
      "voice_settings":{"stability":0.5,"similarity_boost":0.8,"style":0.3,"use_speaker_boost":True,"speed":1.1},
    ...
    open(f"audio/v3/{it['id']}.mp3","wb").write(base64.b64decode(r['audio_base64']))
    ...
    json.dump({"id":it['id'],"text":it['text'],"dauer":round(al['character_end_times_seconds'][-1],3),"woerter":words},open(f"audio/v3/{it['id']}.json","w"),ensure_ascii=False,indent=1)
