...
import {
  fetchBodenrichtwertWithStatus,
  isImBorisGebiet,
  hintFuerObjektart,
  BORIS_QUELLEN,
} from "@/lib/bewertung/boris";

/**
 * Portiert aus dem Riegel-Projekt (src/app/api/bodenrichtwert/route.ts) —
 * unverändert bis auf den Import-Pfad (lib/boris → lib/bewertung/boris).
 *
 * Server-Proxy für den amtlichen Bodenrichtwert (RLP: VBORIS-WMS, Hessen:
 * BORIS-WFS — s. lib/bewertung/boris.ts) — der Rechner ruft ausschließlich
 * diese Route auf, nie die Landesdienste direkt (keine Nutzer-IP an Dritte,
 * zentrale Drossel, ein gemeinsamer Cache in lib/bewertung/boris.ts für
 * Client- und Server-Rechnung/PDF).
 *
 * Grobe Länder-BBoxen (inkl. Toleranzrand, s. lib/bewertung/boris.ts)
 * filtern offensichtlichen Unsinn aus, bevor überhaupt ein externer Dienst
