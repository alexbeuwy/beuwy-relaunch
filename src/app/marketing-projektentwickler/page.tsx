 ...
 *
 * R11: alle seiteneigenen Texte laufen über Studio-Keys
 * (src/lib/texte/seiten/marketing-projektentwickler.ts). Die mk.*-Keys
 * aus content.ts (Floating-Card-Zahl) bleiben unverändert.
 */

export const revalidate = 60;

export async function generateMetadata(): Promise<Metadata> {
  const t = seitenTexte(await getContent(), "marketing-projektentwickler");
  return {
    title: t("meta.titel"),
...
export default async function MarketingProjektentwicklerPage() {
  ...
  const t = seitenTexte(c, "marketing-projektentwickler");
    ...
    <>
        ...
        <div className="relative min-h-[70dvh]">
          <div className="relative mt-4 aspect-[4/3] w-full overflow-hidden lg:absolute lg:inset-y-0 lg:right-0 lg:mt-0 lg:aspect-auto lg:h-full lg:w-[52vw] lg:rounded-bl-[48px]">
            ...
            <div className="absolute bottom-8 left-6 max-w-[13.5rem] rounded-2xl bg-white/95 p-5 backdrop-blur-sm lg:bottom-12 lg:left-10">
              <p className="t-label !text-[10px]">{t("hero.beweis_label")}</p>
      ...
      <section id="problem" className="bg-bg-elevated">
        <div className="mx-auto max-w-[1120px] px-6 py-20 md:py-28 lg:px-10">
          <Reveal>
            <SektionsKopf eyebrow={t("problem.eyebrow")} titel={t("problem.titel")} className="max-w-[720px]" />
          </Reveal>
