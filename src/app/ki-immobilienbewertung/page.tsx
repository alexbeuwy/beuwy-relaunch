 ...
 * R3-SEITENPLAN.json.
 *
 * R11 (14.09): jeder Text läuft über Studio-Keys s.ki-immobilienbewertung.*
 * (src/lib/texte/seiten/ki-immobilienbewertung.ts).
 */

export const revalidate = 60;

export async function generateMetadata(): Promise<Metadata> {
  const t = seitenTexte(await getContent(), "ki-immobilienbewertung");
  return {
    title: t("meta.titel"),
...
export default async function KiImmobilienbewertungPage() {
  const c = await getContent();
  const t = seitenTexte(c, "ki-immobilienbewertung");
  const schritte = t.liste("schritte", ["titel", "text"] as const);
  const pains = t.liste("pains", ["zitat", "antwort"] as const);
  const vergleich = t.liste("vergleich", ["merkmal", "rechner", "gutachten"] as const);
    ...
    <>
      ...
      <section id="fehlerquellen" className="bg-bg-base">
        <div className="mx-auto max-w-[1120px] px-6 py-20 md:py-28 lg:px-10">
          <Reveal>
            <SektionsKopf
              eyebrow={t("fehlerquellen.eyebrow")}
              titel={t("fehlerquellen.titel")}
              className="max-w-[720px]"
            />
          </Reveal>
              ...
              <table className="w-full min-w-[640px] border-collapse text-left">
                <thead>
                  <tr className="border-b border-line-subtle">
                    <th className="t-label py-3 pr-6 font-semibold">{t("vergleich.kopf_merkmal")}</th>
                    <th className="t-label py-3 pr-6 font-semibold">{t("vergleich.kopf_rechner")}</th>
                    <th className="t-label py-3 font-semibold !text-ink-cream">
                      {t("vergleich.kopf_gutachten")}
                    </th>
                  </tr>
                </thead>
                <tbody>
