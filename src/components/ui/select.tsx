  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={
          <RiArrowDownSLine className="pointer-events-none size-4 text-ink-muted" />
        }
      />
    </SelectPrimitive.Trigger>
  )
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn("px-3 py-2.5 text-xs tracking-[0.01em] text-ink-muted", className)}
      {...props}
    />
  )
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2.5 rounded-[8px] py-2 pr-8 pl-3 text-sm font-medium outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
