 ...
 *
 * PORT-HINWEIS (beuwy, 27.08.2026): 1:1 aus dem Riegel-Projekt
 * (src/lib/valuation.ts) übernommen — die Rechenlogik, alle Faktoren,
 * Kalibrierkonstanten und Kommentare sind unverändert (keine
 * Riegel-Markenreferenzen im Original, daher nichts zu neutralisieren).
 * Einzige Anpassung: `stadtFaktorFuerOrt`/`stadtNiveauFuerOrt` lebten im
 * Original in zwei eigenen Dateien (lib/stadt-faktor.ts, lib/stadt-niveau.ts)
 * und wanderten hier — weil der Modul-Vertrag dieses Ports nur die sechs
 * Dateien unter src/lib/bewertung/ vorsieht — unverändert als eigener
 * Abschnitt DIREKT IN DIESE DATEI (s. u., vor REGIONS). Beide Tabellen sind
 * reine bundesweite/regionale Ortsdaten ohne Riegel-Bezug, ihre Logik bleibt
 * exakt wie im Original.
 */
import { ortAusLabel } from "./geocode";

/* ════════════════════════════════════════════════════════════════════
 * Eingefaltet aus dem Riegel-Original src/lib/stadt-faktor.ts (1:1) —
 * s. PORT-HINWEIS oben. Weiterhin GEMEINSAME Quelle für Preisatlas
 * (marktdaten.ts) und Bewertungs-Engine (unten in estimateValue).
 *
 ...
 * dort als Mikrolagen-Korrektiv, s. lageFaktor in estimateValue).
 *
 * Schlüssel sind Orts-Slugs (ursprünglich Riegels Standort-Artikel-Slugs,
 * hier ohne diese Abhängigkeit als reine String-Konstanten) —
 * `stadtFaktorFuerOrt` normalisiert freie Ortsnamen auf dieses Schema.
 *
 * Diese Tabelle deckt AUSSCHLIESSLICH die vormalige Riegel-Kernregion
 * (Vorderpfalz/Rhein-Neckar) ab. Für beuwys bundesweite Zielgruppe bleibt
 * sie als (harmloser) Spezialfall stehen: Orte außerhalb der Liste liefern
 * Faktor 1 (neutral) und rechnen über STADT_NIVEAU bzw. die BRW-Ableitung
 * weiter unten — dieselben Schichten, die ohnehin für die überwiegende
 * Mehrheit der Orte bundesweit greifen.
 * ════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════
 * Eingefaltet aus dem Riegel-Original src/lib/stadt-niveau.ts (1:1) —
 * s. PORT-HINWEIS oben.
 *
 ...
 * REGIONS-Abschlüssen (Schichten-Reihenfolge s. estimateValue weiter
 * unten). Für beuwys Makler-Kundschaft bundesweit liefert sie für die
 * großen Zielstädte sofort ein realistisches Startniveau, statt überall
 * auf den (Vorderpfalz-geprägten) DEFAULT_REGION-Wert zurückzufallen.
 * ════════════════════════════════════════════════════════════════════ */
...
/** Gleiche Normalisierung wie oben bei STADT_FAKTOR (Umlaute, Klammern, Trenner). */
function slugifyOrtNiveau(ort: string): string {
  return ort
    .toLowerCase()
    .replace(/\s*\(([^)]+)\)/g, "-$1")
    .replace(/ä/g, "ae")
...
export function stadtNiveauFuerOrt(ort: string): StadtNiveau | null {
  const slug = slugifyOrtNiveau(ort);
...
export interface ValuationInput {
  ...
  /** Ausweistyp zur Klasse (optional): Bedarfsausweise fallen bauartbedingt
   *  systematisch schlechter aus als Verbrauchsausweise desselben Gebäudes
   *  (rechnerischer Bedarf vs. reales Nutzerverhalten) — der Energie-Malus
   *  wird bei „bedarf" gedämpft, s. Energie-Block in estimateValue. */
 ...
 */
/**
 * Grundstücks-MEHRWERT eines Mehrfamilienhauses im Ertragswert-Ansatz
 * (Hinweis Manfred, 21.08.2026: „Grundstücksgröße fehlt beim MFH").
 *
 * Warum nicht die volle Staffel wie beim Haus: Miete × Vervielfältiger
 * preist das Gebäude MITSAMT einem betriebsnotwendigen Grundstück — die
 * Miete steigt nicht, weil die Parzelle größer ist. Den vollen Bodenwert
 * zusätzlich anzurechnen wäre eine Doppelzählung. Was der Ertragswert aber
 * strukturell übersieht, ist Land ÜBER den Normalbedarf hinaus (Baureserve,
 * zweite Baureihe, großer Garten) — gemessen vor dem Fix: ein 10-WE-Haus auf
 * 2.500 m² wurde exakt wie eines auf 600 m² bewertet.
 *
 * Ansatz: Normalbedarf ≈ Wohnfläche / GFZ 0,8 (konservativ für städtische
 * MFH-Lagen). Nur die Fläche darüber zählt — als Bauland-Reserve zum
 * Mehrflächen-Satz der Haus-Staffel (25 % BRW, erste 700 m²), dahinter als
 * Gartenland. Kleine Überhänge (< 100 m²) werden ignoriert: das ist
 * Messrauschen der Eigentümer-Angabe, keine Baureserve.
 */
const MFH_GFZ_NORMAL = 0.8;
const MFH_MEHRFLAECHE_MIN_M2 = 100;
function mfhGrundstuecksMehrwert(
  grundflaeche: number,
  wohnflaeche: number,
  brw: number,
): { mehrflaecheM2: number; wert: number } {
  const normalbedarf = wohnflaeche / MFH_GFZ_NORMAL;
  const mehr = grundflaeche - normalbedarf;
  if (mehr < MFH_MEHRFLAECHE_MIN_M2) return { mehrflaecheM2: 0, wert: 0 };
  const reserveM2 = Math.min(mehr, 700);
  const gartenM2 = Math.max(mehr - 700, 0);
  return {
    mehrflaecheM2: Math.round(mehr),
    wert: Math.round(reserveM2 * STAFFEL.haus.mehrSatz * brw + gartenM2 * gartenlandSatz(brw)),
  };
}
...
export function estimateValue(input: ValuationInput, opts?: EstimateOptions): ValuationResult {
  ...
  let efRoh = energieklasseEffektiv ? ENERGIE_FACTOR[energieklasseEffektiv] ?? 1.0 : 1.0;
  // Bedarfsausweis-Dämpfung (21.08.2026): Der BEDARFSausweis rechnet die
  // Gebäudephysik unter Normbedingungen und fällt für denselben Bau meist
  // ein bis zwei Klassen schlechter aus als der VERBRAUCHSausweis (reales
  // Heizverhalten). Ein schlechter Bedarfswert drückt den Marktpreis darum
  // weniger als ein gleich schlechter Verbrauchswert — der Malus wird um
  // 30 % gedämpft. Nur den MALUS: gute Klassen aus einem Bedarfsausweis sind
  // eher konservativ und bleiben voll stehen. Greift nur, wenn die Klasse
  // vom Nutzer kam (nicht auf der Altbau-Annahme oben — die ist kein Ausweis).
  if (input.energieausweis === "bedarf" && input.energieklasse && efRoh < 1) {
    efRoh = 1 + (efRoh - 1) * 0.7;
    annahmen.push(
      `Energieklasse ${input.energieklasse} aus einem Bedarfsausweis: Der rechnerische Bedarf fällt bauartbedingt oft schlechter aus als der reale Verbrauch — der Energie-Abschlag ist deshalb um 30 % gedämpft.`,
    );
  }
  ...
  if (input.objektart === "grundstueck") {
    ...
    // Übergroßes Grundstück auf den Ertragswert aufschlagen (Hinweis Manfred
    // 21.08.2026): nur die Fläche ÜBER dem Normalbedarf zählt — Begründung und
    // Sätze s. mfhGrundstuecksMehrwert. Vor dem Vergleichswert-Anker, damit
    // beide Seiten des Vergleichs ein Grundstück eingepreist haben (der
    // Vergleichswert trägt seine eigene volle Staffel).
    if (input.grundflaeche && wf > 0) {
      const gm = mfhGrundstuecksMehrwert(input.grundflaeche, wf, boden);
      if (gm.wert > 0) {
        mid += gm.wert;
        annahmen.push(
          `Übergroßes Grundstück: ${Math.round(input.grundflaeche).toLocaleString("de-DE")} m² bei ${Math.round(wf).toLocaleString("de-DE")} m² Wohnfläche — die ${gm.mehrflaecheM2.toLocaleString("de-DE")} m² über dem baulichen Normalbedarf sind als Reserve-/Gartenfläche mit ${gm.wert.toLocaleString("de-DE")} € zusätzlich zum Ertragswert angesetzt (die Miete bildet nur das Gebäude samt Normalgrundstück ab).`,
        );
      }
    }
    ...
    if (ankerGewicht > 0 && wf > 0) {
      ...
      if (ankerBoden > mid) {
        if (ankerGewicht >= 1) {
          ...
          // Der Übergroß-Aufschlag galt dem Ertragswert — der ist jetzt vom
          // Vergleichswert (mit eigener voller Grundstücks-Staffel) abgelöst.
          // Die Annahme dazu wäre irreführend und fliegt wieder raus.
          const uebergrossIdx = annahmen.findIndex((a) => a.startsWith("Übergroßes Grundstück:"));
          if (uebergrossIdx >= 0) annahmen.splice(uebergrossIdx, 1);
